...
│       └── src/schemas/         # Zod schemas — cta.schema.ts (ctas max 3), heroSlide.schema.ts
├── infra/
...
| Zod | 3.x | runtime schema for CMS `ctas[]` max 3 (hero/banner) + shared contract validation (`shared/src/schemas/`) |
...
- Zod validation for CMS content (`shared/src/schemas/cta.schema.ts` — `ctas[]` max 3) before publish
