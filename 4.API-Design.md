...
> REST API Contracts for All 9 Microservices (polyrepo — 1 repo per service, see `2.Tech-Stack.md §3`)
> Status: Draft | Last Updated: 2026-09-03
> Parent: `0.Project-Overview.md` | Siblings: `1.Architecture.md` (dual transport), `3.Database-Schema.md` | Contracts: `@ecom/contracts` (semver `8.Versioning.md`)
...
### 1.8 Contract-First & OpenAPI (via `@ecom/contracts`)
- Source of truth is **`ecom-contracts` repo (`@ecom/contracts`)** — event envelopes, 10 payload schemas, REST DTOs, Zod `cta.schema.ts` (`ctas[]` max 3), error codes, topic/queue map. This doc references; services pin a version (no `^`). Breaking changes follow `8.Versioning.md` (MAJOR = dual-publish window + CHANGELOG).
- Full OpenAPI 3.1 specs generated per service **from `@ecom/contracts` DTOs** (Swagger module), exposed at `GET /api/v1/{route}/docs` and exported to `zod-to-openapi`.
- Event payload contracts live in `1.Architecture.md` §7 (transport-agnostic) + `@ecom/contracts/src/events/` — API doc only references them.
- Transport note: **local `TRANSPORT=kafka` supports log replay** (e.g. re-consume `product.imported` after search bug); **AWS `TRANSPORT=sqs` deletes on consume** (max 14d retention) — replay requires S3 archive or DB scan. Documented in `1.Architecture.md §6.5`.
- SQS consumer hygiene (AWS): **long-poll 20s + batch 10 + FIFO where ordered (`cart.updated`/`order.created` per-aggregate order)`** — avoids empty-receive inflation ($0.40/M Standard / $0.50/M FIFO, first 1M/mo free, 64KB chunk rule).
...
> Legend: 🛡 = requires auth (customer), 👑 = admin role, ✳ = emits event via `IEventBus` — **Kafka locally (`TRANSPORT=kafka`) / SNS→SQS on AWS (`TRANSPORT=sqs`)** (→ §6, `1.Architecture.md §6.5`). Envelope from `@ecom/contracts`.
...
> `content` is Zod-validated via `@ecom/contracts/src/schemas/cta.schema.ts` — hero `content.slides[].ctas[]` max 3 and banner `content.ctas[]` max 3; `ctas.length > 3` → `400 VALIDATION_FAILED { field: "content.slides[0].ctas", message: "max 3 CTAs per slide" }`.
...
> Typeahead + ranked search over the unified index (products + categories + blogs). Index synced via events (`product.imported`, `product.updated`, `category.updated`, `blog.updated`) over `IEventBus` (Kafka local / SNS→SQS AWS) — reads only here, no writes. Replay on AWS unavailable (S3 archive or DB scan instead).
| Method | Path | Auth | Request | Response | Events |
|---|---|---|---|---|---|
...
## 6. Event ↔ Endpoint Traceability (transport-agnostic — `IEventBus`, see `1.Architecture.md §6`)

| Event (from `1.Architecture.md` §7) | Produced by | Via endpoint/flow | Bus (local / AWS) |
|---|---|---|---|
| `cart.updated` | cart-svc | `POST/PATCH/DELETE /cart/items`, batch remove, clear, merge | Kafka topic 3pt (key `userId\|sessionId`) / **FIFO SQS** `MessageGroupId=userId\|sessionId` (order preserved) |
| `cart.abandoned` | analytics-svc | Detector consumer (no REST origin) | FIFO SQS |
| `order.created` | order-svc | `POST /order/checkout` → outbox relay | Standard SQS via SNS (one queue per consumer) / FIFO if per-order order required |
| `order.status.updated` | order-svc | `PATCH /orders/:id/status`, `POST /orders/:id/refund` | Standard via SNS |
| `product.imported` | import-svc | `POST /import/import` (per valid row) | Standard via SNS — **no replay on SQS** (vs Kafka replay) |
| `page.viewed` | analytics-svc | `POST /analytics/events/page-view` (via `/api/track`) | **Standard + batch 10 + long-poll 20s** (high volume, order irrelevant) |
| `user.registered` | auth-svc | `POST /auth/register` | Standard via SNS |
| `cms.updated` | cms-svc | Section/settings/navigation/page writes (§3.5 ✳) | Standard via SNS |
| `product.updated` | catalog-svc | `PATCH /products/:id`, variant/option writes | Standard via SNS |
| `category.updated` | catalog-svc | Category create/update (admin) | Standard via SNS |
| `blog.updated` | cms-svc | Blog create/update/publish (admin) | Standard via SNS |

Consumers per event: see `1.Architecture.md` §6.4. Search index sync consumers: `product.imported`, `product.updated`, `category.updated`, `blog.updated` → search-svc. AWS DLQ: SQS redrive `maxReceiveCount 5` → DLQ queue (vs `.dlq` topic local); SQS bills $0.40/M Standard / $0.50/M FIFO (first 1M free, 64KB chunks).
...
| search-svc | — (`IEventBus` only) | consumes `product.imported` / `product.updated` / `category.updated` / `blog.updated` via Kafka (local) / SNS→SQS (AWS) | Index sync — no REST deps; replay unavailable on SQS |
...
- `1.Architecture.md` — event contracts, dual-transport topology (Kafka local / SNS→SQS AWS), Nginx routing
- `2.Tech-Stack.md` — polyrepo layout, `@ecom/contracts` bus abstraction (Next 16 + React 19)
