...
- This doc = contract source of truth. Full OpenAPI 3.1 specs generated later **from NestJS DTOs** (Swagger module), one spec per service, exposed at `GET /api/v1/{route}/docs`.
...
| POST | `/sections` | 👑 | `{key, type, pageKey, content, settings?}` | `201 {section}` | `cms.updated` ✳ |
| PUT | `/sections/:key` | 👑 | full section body | `{section}` | `cms.updated` ✳ |
...
{
  "sections": [
    { "key": "home-hero", "type": "hero", "content": { "imageUrl": "...", "title": "Summer Sale", "cta": { "text": "Shop now", "href": "/products" } }, "settings": { "ratio": "16:9", "blendMode": "multiply" }, "sortOrder": 0, "isActive": true }
  ],
  "version": 7
}
```
`version` = global settings cache-buster; storefront caches by version.
